...
@section('title')Tipo de Imagenes @endsection

@section('breadcrumb')
<h1 class="m-0 text-dark">Tipo de Imagenes</h1>
<ol class="breadcrumb">
    <li><a href="javascript: void(0);"><i class="fa fa-dashboard"></i> CMS OK COMPUTER</a></li>
    <li>Diseño</li>
    <li class="active">Tipo de Imagenes</li>
</ol>
@endsection
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table class="table table-condensed table-bordered table-hover table-striped" id="tableArtType">
                        <thead>
                            <tr>
                                <th>Nombre</th>
                                <th width="50">&nbsp;</th>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#tableArtType").DataTable({
                ...
                language: idioma,
                ajax: {
                    url: "{{ route('lists.list-arts-type') }}",
                    type: "GET",
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            var store = "{{ route('catalogs.arts-type.store') }}";
                            $("#formulario")[0].reset();
                            $("#formulario").attr("action", store);
                            $("[name=_method]").val("POST");
                            $("#modal-data .modal-title").text("Agregar nuevo tipo");
                            $("#btnAction").text("Grabar nuevo tipo");
            ...
            $("#tableArtType tbody").on("click", "a.edit_dt", function () {
                showData($(this).data("route"), $(this).data("update"));
            });

            $("#tableArtType tbody").on("click", "a.delete_dt", function () {
        ...
        function showData(route, update) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#modal-data .modal-title").text("Editar tipo ["+ response.name +"]");
        ...
        function deleteData(route){
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#tableArtType").DataTable().ajax.reload();
                }
            }).fail( function (jqXHR, textStatus, errorThrown) {
                console.log(jqXHR);
                console.log(textStatus);
