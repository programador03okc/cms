...
@section('title')Etiqueta de Productos @endsection

@section('breadcrumb')
<h1 class="m-0 text-dark">Etiqueta de Productos</h1>
<ol class="breadcrumb">
    <li><a href="javascript: void(0);"><i class="fa fa-dashboard"></i> CMS OK COMPUTER</a></li>
    <li>Configuración</li>
    <li class="active">Etiqueta de Productos</li>
</ol>
@endsection
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table class="table table-condensed table-bordered table-hover table-striped" id="tableTag">
                        <thead>
                            <tr>
                                <th>Nombre</th>
                                <th width="50">&nbsp;</th>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            var store = "{{ route('configurations.tag.store') }}";
                            $("#formulario")[0].reset();
                            $("#formulario").attr("action", store);
                            $("[name=_method]").val("POST");
                            $("#modal-data .modal-title").text("Agregar nueva etiqueta");
                            $("#btnAction").text("Grabar nueva etiqueta");
            ...
            $("#tableTag tbody").on("click", "a.edit_dt", function () {
                showData($(this).data("route"), $(this).data("update"));
            });

            $("#tableTag tbody").on("click", "a.delete_dt", function () {
        ...
        function showData(route, update) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#modal-data .modal-title").text("Editar etiqueta ["+ response.name +"]");
