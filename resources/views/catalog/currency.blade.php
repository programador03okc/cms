        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table class="table table-condensed table-bordered table-hover table-striped" id="tableCurrency">
                        <thead>
                            <tr>
                                <th width="90">Simbol</th>
                                <th>Nombre</th>
                                <th width="50">&nbsp;</th>
                            </tr>
                        </thead>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <div class="form-group">
                                <h6>Símbolo</h6>
                                <input type="text" class="form-control input-sm" name="simbol" placeholder="Ejm: SOL" maxlength="5" required>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            var store = "{{ route('catalogs.currency.store') }}";
                            $("#formulario")[0].reset();
                            $("#formulario").attr("action", store);
                            $("[name=_method]").val("POST");
                            $("#modal-data .modal-title").text("Agregar nueva moneda");
                            $("#btnAction").text("Grabar nueva moneda");
            ...
            $("#tableCurrency tbody").on("click", "a.edit_dt", function () {
                showData($(this).data("route"), $(this).data("update"));
            });

            $("#tableCurrency tbody").on("click", "a.delete_dt", function () {
        ...
        function showData(route, update) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#modal-data .modal-title").text("Editar moneda ["+ response.name +"]");
